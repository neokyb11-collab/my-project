...
export default function Settings() {
  ...
  const [preferredLlm, setPreferredLlm] = useState<"claude" | "gemini">("gemini");
  ...
  useEffect(() => {
    if (getSettingsQuery.data) {
      ...
      setPreferredLlm((getSettingsQuery.data.preferredLlm as "claude" | "gemini") ?? "gemini");
    }
      ...
      <div className="max-w-2xl mx-auto px-4 py-8">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
            <h2 className="text-base font-semibold text-slate-900 mb-1">자막 생성 AI</h2>
                ...
                >
                  <div className="flex items-center justify-between mb-1">
                    <span className="font-semibold text-slate-900">
                      {llm === "claude" ? "Claude" : "Gemini"}
                    </span>
                    {preferredLlm === llm && <CheckCircle2 className="w-4 h-4 text-blue-500" />}
                  </div>
                  <p className="text-xs text-slate-500">
                    {llm === "claude" ? "Anthropic Claude Sonnet" : "Google Gemini 2.5 Flash"}
                  </p>
                </button>
              ))}
            </div>
          ...
          {/* Deepgram API Key */}
          <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
            <h2 className="text-base font-semibold text-slate-900 mb-1">Deepgram API 키 <span className="text-red-500">*필수</span></h2>
            <p className="text-sm text-slate-500 mb-4">
              음성 인식에 사용됩니다. 무료 200시간/월 제공.{" "}
