...
function getPool(): mysql.Pool | null {
  if (!_pool && process.env.DATABASE_URL) {
    try {
      ...
      console.warn("[Database] 풀 생성 실패:", error);
    }
  }
  return _pool;
}
...
export async function getConnection(): Promise<mysql.PoolConnection | null> {
  ...
  try {
    ...
    console.warn("[Database] 연결 실패:", error);
    return null;
  }
}
...
export async function createSubtitleProject(
  ...
  if (!connection) throw new Error("데이터베이스를 사용할 수 없습니다");
...
export async function upsertUserSettings(
  ...
  try {
    await connection.execute(
      ...
      [userId, claudeApiKey ?? null, geminiApiKey ?? null, deepgramApiKey ?? null, preferredLlm ?? "gemini", preferredTranscriber ?? "whisper", now, now]
    );
  } finally {
    connection.release();
  }
